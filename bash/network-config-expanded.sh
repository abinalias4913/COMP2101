...
# normally default route can be found in routing table
# genetent used for obtaining router names
...
# second part of the output generates a per-interface report
...
for interface in $interfaces_list; do
    ...
    # Identify network number for this interface and the name if it has one
...
#####
# Ending the per-interface report
#####
